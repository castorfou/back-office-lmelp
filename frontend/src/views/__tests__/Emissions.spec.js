...
describe('Emissions.vue - Avis Matching Stats', () => {
  let wrapper;
  let router;
  ...
  it('should reload matching_stats after reextractAvis (Issue #185)', async () => {
    // Mock emissions list
    emissionsService.getAll.mockResolvedValue([
      {
        _id: 'emission123',
        date: '2025-09-14T10:59:39',
        episode_id: 'episode123',
      },
    ]);

    // Mock initial avis with old stats (before reextract)
    avisService.getAvisByEmission.mockResolvedValueOnce({
      avis: [
        {
          id: 'avis1',
          livre_titre_extrait: 'La Maison vide',
          match_phase: null, // Pas encore matché
        },
      ],
      matching_stats: {
        livres_summary: 1,
        ...
        match_phase3: 0,
        match_phase4: 0,
        unmatched: 1, // 1 livre non matché
      },
    });
    ...
    // Set emission ID
    wrapper.vm.selectedEmissionId = 'emission123';
    await wrapper.vm.$nextTick();

    // Mock reextract result (new match)
    avisService.extractAvis.mockResolvedValue({
      extracted_count: 1,
      matching_stats: {
        livres_summary: 1,
        livres_mongo: 1,
        match_phase1: 1, // Maintenant matché en phase 1
        match_phase2: 0,
        match_phase3: 0,
        match_phase4: 0,
        unmatched: 0, // Plus de non-matché
      },
    });

    // Mock reload after reextract with NEW stats
    avisService.getAvisByEmission.mockResolvedValueOnce({
      avis: [
        {
          id: 'avis1',
          livre_titre_extrait: 'La Maison vide',
          match_phase: 1, // Matché en phase 1
        },
      ],
      matching_stats: {
        livres_summary: 1,
        livres_mongo: 1,
        match_phase1: 1, // Stats mises à jour
        match_phase2: 0,
        match_phase3: 0,
        match_phase4: 0,
        unmatched: 0,
    ...
    // Trigger reextractAvis
    await wrapper.vm.reextractAvis();
    await wrapper.vm.$nextTick();

    // CRITICAL: Verify matching_stats are reloaded (Issue #185 fix)
    expect(wrapper.vm.avisMatchingStats).toEqual({
      livres_summary: 1,
      livres_mongo: 1,
      match_phase1: 1, // ← Devrait être 1 (matché), pas 0
      match_phase2: 0,
      match_phase3: 0,
      match_phase4: 0,
      unmatched: 0, // ← Devrait être 0, pas 1
    });

    // Verify stats changed from initial state
    expect(wrapper.vm.avisMatchingStats.match_phase1).toBe(1);
    expect(wrapper.vm.avisMatchingStats.unmatched).toBe(0);
  });
});
